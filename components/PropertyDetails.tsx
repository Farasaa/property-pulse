...
export default function PropertyDetails( {property}: {property: PropertyCard}) {
    const {
        type,
        name,
        location,
          ...
          <div className="flex flex-col md:flex-row justify-around">
            ...
            >
              ...
              <div className="text-2xl font-bold text-blue-500">
                {rates.nightly ? `$${rates.nightly}` : <FaTimes className="text-red-700" />}
              </div>
            </div>
            <div
              className="flex items-center justify-center mb-4 border-b border-gray-200 md:border-b-0 pb-4 md:pb-0"
            >
              ...
              <div className="text-2xl font-bold text-blue-500">{rates.weekly ? `$${rates.weekly}` : <FaTimes className="text-red-700" />}</div>
            </div>
            <div className="flex items-center justify-center mb-4 pb-4 md:pb-0">
              <div className="text-gray-500 mr-2 font-bold">Monthly</div>
              <div className="text-2xl font-bold text-blue-500">{rates.monthly ? `$${rates.monthly}` : <FaTimes className="text-red-700" />}</div>
            </div>
          </div>
        </div>
